...
class Visualizer extends Sprite {
	...
	var barsA:Array<Quad>;
	var barsB:Array<Quad>;
	var barsC:Array<Quad>;

	var fft:FFT;
	var sound:AudioSource;
    var current:Array<Quad>;
	...
	private function onEnterFrame(event:EnterFrameEvent):Void {
		if (sound != null) {
			...
			for (i in 0...256) {
				...
				value = Math.min(180, value); // Cap the value to prevent overflow
				current[i].height = value;
				current[i].y = 190 - value; // Adjust y-position based on the bar height
			}
		}

        if(current == barsA){
            current = barsB;
        } else if(current==barsB){
            current = barsC;
        } else  if(current==barsC){
            current = barsA;
        }
	}
